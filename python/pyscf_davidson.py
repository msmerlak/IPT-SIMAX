...
#
# Modified from https://github.com/pyscf/pyscf/blob/master/pyscf/lib/linalg_helper.py
# by Qiming Sun <osirpt.sun@gmail.com>
...
def davidson(aop, x0, precond, tol=1e-12, max_cycle=50, max_space=12,
    ...
    '''
    e, x, its, timing = davidson1(lambda xs: [aop(x) for x in xs],
                     x0, precond, tol, max_cycle, max_space, lindep,
                     max_memory, dot, callback, nroots, lessio, pick, verbose,
                     follow_state)[1:]
    if nroots == 1:
        return {'iterations':its, 'time' : timing, 'eigenvalue': e[0], 'eigenvector' : x[0], 'residual': np.linalg.norm(aop(x[0]) - e[0]*x[0])/np.linalg.norm(x[0])}
    else:
        return {'iterations':its, 'time' : timing, 'eigenvalues':e, 'eigenvectors' : x, 'residual': np.linalg.norm(aop(x) - e*x)/np.linalg.norm(x)}
...
# but the eigenvectors x0 might not be strictly orthogonal
    ...
    return numpy.asarray(conv), e, x0, its, timing
...
def eigs_davidson(s, v0):
    op = lambda x: s.dot(x)
    e, c = linalg_helper.davidson(op, v0, s.diagonal())
    return(e,c)
