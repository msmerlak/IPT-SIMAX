...
def eigs_slepc(A, k, *, B=None, which=None, sigma=None, isherm=True, P=None,
               ...
               l_win=None, comm=None, quiet = True):
...
def extract_result(eigensolver, k, ):
    """Solve a matrix using the advanced eigensystem solver"""
    ...
    if not quiet:
        ...
        print("Time to solve: %f" % timing)

    eigensolver.destroy()
    return {'iterations': its, 'time': timing, 'eigenvalue': lk[0], 'eigenvector': vk, 'residual' : np.linalg.norm(A @ vk - lk * vk)/np.linalg.norm(vk)}
