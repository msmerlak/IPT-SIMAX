...
%DIF ADD main-revision3.tex   Tue Mar 22 15:31:22 2022
...
\DIFdelbegin \DIFdel{We use some elementary notions from projective and differential geometry. Their definitions are given below.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Informally, an $(n-1)$-dimensional projective space $\mathbb{C}P^{n-1}$ is the space of complex lines (1-dimensional complex subspaces) of the vector space $\mathbb{C}^n$ (the notion is generalizable to vectors spaces over fields different from $\mathbb{C}$). It is constructed in the following way. Consider an equivalence relation of }\emph{\DIFdel{nonzero}} %DIFAUXCMD
\DIFdel{vectors $\mathbf{v},\mathbf{w}\in\mathbb{C}^n$ given by $\mathbf{v} \sim \mathbf{w}$ if $\exists \alpha \in \mathbb{C}$ such that $\mathbf{v} = \alpha \mathbf{w}$. In other words, all collinear vectors, with exception of $0$, are declared to be equivalent.
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{The }\emph{\DIFdel{projective space}} %DIFAUXCMD
\DIFdel{$\mathbb{C}P^{n-1}$ is the factor-space $(\mathbb{C}^n\setminus\{0\})/{\sim}$ with the associated factor-topology.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As usual, we denote the equivalence class of a vector $\mathbf{z}\in\mathbb{C}^n$ (a point in $\mathbb{C}P^{n-1}$) as $[\mathbf{z}]$, and $\mathbf{z}$ is called a representative of that class. The coordinates of $\mathbb{C}^n$ can be used to (nonuniquely) parametrize points of $\mathbb{C}P^{n-1}$ in the following way.
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{A tuple $[z_1:z_2:\dots:z_n]$ is called }\emph{\DIFdel{homogeneous coordinates}} %DIFAUXCMD
\DIFdel{of a point $p\in\mathbb{C}P^{n-1}$ if there is a vector $\mathbf{z}\in\mathbb{C}^n$ such that $p = [\mathbf{z}]$ and $\mathbf{z}$ has coordinates $(z_1,z_2,\ldots,z_n)$.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Thus each point of the projective space has a set of homogeneous coordinates, but all of them are related by homogeneous rescaling: two homogeneous coordinates of the same point $[x_1:\dots:x_n]$ and $[y_1:\dots:y_n]$ are related by $x_i = \alpha y_i$ with the same $\alpha$ for each $i$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{$\mathbb{C}P^{n-1}$ has a structure of a complex $(n-1)$-dimensional manifold.
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{A }\emph{\DIFdel{complex (coordinate) chart}} %DIFAUXCMD
\DIFdel{on a topological space $M$ is a homeomorphism $\phi\colon V\to O\in\mathbb{C}^n$ from some open set $V \subset M$ to an open set $O$ of $\mathbb{C}^n$ with the standard topology for some dimension $n$.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{It is convenient to denote a chart $\phi$ defined on $V\subset M$ with $(V,\phi)$ explicitly marking its domain.
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{Let there be two complex charts $(V,\phi)$ and $(W,\psi)$, $V,W \subset M$, and let their ranges have the same dimensionality $n$. The homeomorphism $\phi\circ\psi^{-1}$ between $\psi(V\cap W) \subset \mathbb{C}^n$ and $\phi(V\cap W) \subset \mathbb{C}^n$ is called a }\emph{\DIFdel{transition function}} %DIFAUXCMD
\DIFdel{(between charts $\psi$ and $\phi$).
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{A topological space $M^n$ is called a }\emph{\DIFdel{complex $n$-dimensional manifold}} %DIFAUXCMD
\DIFdel{if 1) there is a set of charts $\{(U_i,\phi_i)\}_{i\in I}$ ($I$ is some index set) of $M^n$ such that $\bigcup_{i\in I} U_i = M^n$, 2) all the ranges of $\phi_i$ have the same dimensionality $n$ and 3) all the transition functions $\phi_i\circ\phi_j^{-1}$ are holomorphic functions between open subset of $\mathbb{C}^n$. The system $\{(U_i,\phi_i)\}_{i\in I}$ is called a }\emph{\DIFdel{holomorphic atlas}} %DIFAUXCMD
\DIFdel{on $M^n$.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{An atlas allows to work with a manifold using usual methods of linear algebra and analysis in $\mathbb{C}^n$. The standard atlas of charts that gives $\mathbb{C}P^{n-1}$ a structure of an $(n-1)$-dimensional complex manifold is given by $n$ special charts. For each $i$, $1 \leq i \leq n$, define $U_i = \{[\mathbf{z}]:z_i \neq 0\}$, label coordinates of $\mathbb{C}^{n-1}$ by $\zeta_1$, \ldots, $\zeta_{i-1}$, $\zeta_{i+1}$, \ldots, $\zeta_n$ ($i$-th label is omitted) and set $\zeta_k = z_k/z_i$, which defines a map $\phi_i\colon U_i\to\mathbb{C}^{n-1}$. This defines a holomorphic (indeed rational) chart on $\mathbb{C}P^{n-1}$ with the whole of $\mathbb{C}^{n-1}$ as its image. It is easy to see that $\{U_i,\phi_i\}_{1\leq i\leq n}$ forms a holomorphic atlas of $\mathbb{C}P^{n-1}$. The transition functions from $\phi_i$ with its range coordinatized by $\zeta_k$ and $\phi_j$ with its range coordinatized by $\eta_k$ (with the same labeling omission rule for each chart as above) is given by $\eta_k = \zeta_k/\zeta_j$ for $k \neq i,j$ and $\eta_i = 1/\zeta_j$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In fact, the action of $\phi_i$ on $[\mathbf{z}]$ can be understood as setting $z_i = 1$, rescaling the rest of $z_k$ accordingly and taking the whole tuple $(z_1,\ldots,z_n)$ as coordinates of a point in the original space $\mathbb{C}^n$. The image of the chart $(U_i,\phi_i)$ becomes an $(n-1)$-dimensional affine subspace of the original space. In the following we will abuse notations by denoting the domain of the $i$-th chart, the chart itself, and the corresponding affine subspace with the same symbol $U_i$. We will refer to these special charts as }\emph{\DIFdel{affine charts}}%DIFAUXCMD
\DIFdel{. Geometric relationships between $U_i$ and the action of the transition functions as projections between them is schematically shown in }%DIFDELCMD < \cref{sketch} %%%
\DIFdel{(other notations on this sketch will be introduced below).
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{Let $\{P_k\}_k$ be a system of polynomials in $n$ variables $\{z_i\}_i$. The subset $V = \{\mathbf{z} \in \mathbb{C}^n: \forall\,k\,P_k(z_1,\dots,z_n) = 0\}$ is called an }\emph{\DIFdel{algebraic variety}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{A polynomial $P$ such that $\forall \alpha \in \mathbb{C}$ $P(\alpha z_1,\dots,\alpha z_n) = \alpha^k P(z_1,\dots,z_n)$ is called a }\emph{\DIFdel{homogeneous polynomial}}%DIFAUXCMD
\DIFdel{. It is obvious that if a point $\mathbf{z}$ is a root of a homogeneous polynomial, then every representative of its equivalence class $[\mathbf{z}]$ is a root, too. Thus homogeneous polynomials can be used to define subsets in projective space.
}%DIFDELCMD < 

%DIFDELCMD < \begin{definition}
%DIFDELCMD < %%%
\DIFdel{Let $\{P_k\}_k$ be a system of homogeneous polynomials in $n$ variables $\{z_i\}_i$. The subset $V = \{[\mathbf{z}] \in \mathbb{C}P^{n-1}: \forall\,k\,P_k(z_1,\dots,z_n) = 0\}$ is called a }\emph{\DIFdel{projective variety}}%DIFAUXCMD
\DIFdel{.
}%DIFDELCMD < \end{definition}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Note that any projective variety defines an algebraic variety in each affine chart $U_i$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Eigenvectors as fixed points}\label{fixed-points}
...
\DIFdelbegin \DIFdel{Being defined up to a multiplicative constant, the eigenvectors of matrix $M$ are naturally identified with elements of the complex projective space $\mathbb{C}P^{n-1}$. From this point of view, the system $\{\mathcal E_{ij}\}_{i,j}$ defines a projective variety. }\DIFdelend \DIFaddbegin \DIFadd{Consider special affine subspaces $U_i$ defined by a fixation of the $i$-th coordinate to $z_i=1$. As eigenvectors are distinguished only up to rescaling, any eigenvector in general position with a subspace $U_i$ (that is, not parallel to it) is essentially represented as a point in it. A generic matrix $M$ has all its eigenvectors in general position with $U_i$ for each $i$.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend Fix an index $i$. To find the full set of eigenvectors in a single \DIFdelbegin \DIFdel{chart }\DIFdelend \DIFaddbegin \DIFadd{affine subspace }\DIFaddend $U_i$ it is enough to solve a smaller set of equations.

\begin{lemma}\label{subsystem-equivalence}
Eigenvectors $\mathbf z$ of $M$ \DIFdelbegin \DIFdel{such that $[\mathbf z] \in U_i$ are }\DIFdelend \DIFaddbegin \DIFadd{that are not parallel to $U_i$ are }\DIFaddend in one-to-one correspondence with the solutions of the system of equations $\{\mathcal E_{ij}\}_{j:j\neq i}$ \DIFdelbegin \DIFdel{in the same chart}\DIFdelend \DIFaddbegin \DIFadd{that are not parallel to $U_i$}\DIFaddend .
\end{lemma}

\begin{proof}
We need to prove that for \DIFdelbegin \DIFdel{$[\mathbf{z}] \in U_i$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{z}$ shuch that $z_i \neq 0$ }\DIFaddend the two following propositions are equivalent: $(i)$ $\mathbf{z}$ is a root of $(M\mathbf{z})_j z_i = (M\mathbf{z})_i z_j$ for $j \neq i$ and $(ii)$ $\exists \eigval \in \mathbb C$ $M\mathbf{z} = \eigval \mathbf{z}$.

The $(i)$ $\Rightarrow$ $(ii)$ direction. \DIFdelbegin \DIFdel{For $[\mathbf{z}] \in U_i$ it }\DIFdelend \DIFaddbegin \DIFadd{It }\DIFaddend is enough to consider \DIFdelbegin \DIFdel{only vectors }\DIFdelend \DIFaddbegin \DIFadd{a rescaled $\mathbf{z}$ }\DIFaddend with $z_i = 1$. \DIFdelbegin \DIFdel{Then from }\DIFdelend \DIFaddbegin \DIFadd{From }\DIFaddend $\{\mathcal E_{ij}\}_{j:j\neq i}$ we conclude that $(M\mathbf{z})_j = (M\mathbf{z})_i z_j$ for all $j \neq i$. If we now denote the common factor in these expressions as $\eigval = (M\mathbf{z})_i$, then $(ii)$ immediately follows.

The $(ii)$ $\Rightarrow$ $(i)$ direction. \DIFdelbegin \DIFdel{Consider an eigenvector $\mathbf{z}$ of $M$ such that $z_i \neq 0$. From the }\DIFdelend \DIFaddbegin \DIFadd{From the $i$-th component of the }\DIFaddend eigenvalue equation $M\mathbf{z}=\eigval \mathbf{z}$ we can express its eigenvalue as $\eigval=(M \mathbf{z})_i/z_i$; inserting this back into $M \mathbf{z}=\eigval \mathbf{z}$ shows that $\mathbf{z}$ is a root of the system $\{\mathcal E_{ij}\}_{j:i\neq j}$.
\end{proof}

\DIFdelbegin \DIFdel{Denoting $V_i$ the projective variety defined by the system $\{\mathcal E_{ij}\}_{j:j\neq i}$ for a fixed $i$ and recalling that the set $\{U_i\}_i$ forms an atlas we conclude that the set of eigenvectors of $M$ can be identified with the set $\cup_i(V_i\cap U_i)$, which is in fact the projective variety $\cap_i V_i$. Since eigenvectors generically have non-zero coordinates in all directions, each component $V_i\cap U_i$ typically contains the complete set of eigenvectors.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend It should be noted, however, that \DIFdelbegin \DIFdel{$V_i$ taken alone may have points }\DIFdelend \DIFaddbegin \DIFadd{the full set of solutions to the system $\{\mathcal E_{ij}\}_{j:i\neq j}$ may have vectors }\DIFaddend unrelated to the eigenvectors of $M$, but these additional \DIFdelbegin \DIFdel{points }\DIFdelend \DIFaddbegin \DIFadd{vectors }\DIFaddend can only be \DIFdelbegin \DIFdel{at infinity in }\DIFdelend \DIFaddbegin \DIFadd{parallel to }\DIFaddend $U_i$ by Lemma~\ref{subsystem-equivalence}. Indeed, consider \DIFdelbegin \DIFdel{points of $V_i$ }\DIFdelend \DIFaddbegin \DIFadd{solutions to the system $\{\mathcal E_{ij}\}_{j:i\neq j}$ }\DIFaddend such that $z_i = 0$. At the very least they include points that are given by $(M\mathbf{z})_i = 0$ in addition to $z_i = 0$. Thus, in general, there is a whole (\DIFdelbegin \DIFdel{$n-3$)-dimensional complex projective subspace of such solutions in $\mathbb CP^{n-1}$ (corresponding to a (}\DIFdelend $n-2$)-dimensional complex hyperplane in \DIFdelbegin \DIFdel{the affine space }\DIFdelend $\mathbb C^n$ \DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{made of such solutions}\DIFaddend .
...
\DIFdelbegin \DIFdel{projective-geometric structure outlined above, for instance by using charts on the complex projective space other than $z_i=1$}\DIFdelend \DIFaddbegin \DIFadd{conditions $z_i=1$ for specifying affine subspaces $U_i$ and to choose other affine subspaces}\DIFaddend , which would lead to different maps with
...
\begin{proof}
By Lemma~(\ref{lemma-eigenspace}), it is enough to assume that $M$ does not have geometrically multiple eigenvalues and all its eigenvectors are \DIFdelbegin \DIFdel{isolated projective points }\DIFdelend \DIFaddbegin \DIFadd{represented by isolated points in all affine subspaces $U_i$}\DIFaddend , so all fixed points of $F$ are isolated matrices. Let such $M$ be defective. Let $J$ be a Jordan normal form of $M$ and $S$ be the transition matrix that turns $M$ into $J$. Consider a deformation $J_{\mathbf d}$ of $J$ given by $J_{\mathbf d} = J + \diag(\mathbf d)$, where $\mathbf d \in \mathbb C^n$. This induces a deformation of $M$ given by $M_{\mathbf d} = M + S\diag(\mathbf d) S^{-1}$. It is clear that one can find $\mathbf d$ arbitrarily close to 0 (in the standard topology of $\mathbb C^n$) such that all diagonal entries of $J_{\mathbf d}$ are different. As $J_{\mathbf d}$ is upper triangular, its eigenvalues (and thus those of $M_{\mathbf d}$) are equal to its diagonal entries. But then $M_{\mathbf d}$ has a full eigenbasis of isolated eigenvectors. The formerly merged eigenvectors split into distinct ones. Furthermore, the new eigenvectors that split from an old defective eigenvector can be made arbitrarily close to it (as \DIFdelbegin \DIFdel{projective points }\DIFdelend \DIFaddbegin \DIFadd{points of any affine subspace $U_i$ }\DIFaddend in the standard topology of $\mathbb CP^{n-1}$) by choosing $\mathbf d$ close enough to 0. This is obvious from considering the eigenvectors spawned from a single block of $J_{\mathbf d}$ and their behavior at the limit $\mathbf d \to 0$.
...
The theorem guarantees that the eigenvector corresponding to an eigenvalue with multiplicity one is a differentiable function of the parameter as a \DIFdelbegin \DIFdel{projective space valued function }\DIFdelend \DIFaddbegin \DIFadd{vector function pointing on the corresponding point in any affine subspace $U_i$}\DIFaddend .
