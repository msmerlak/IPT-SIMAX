...
%DIF DEL submission/main-revision.tex       Thu Apr 21 12:15:34 2022
%DIF ADD resubmission/without-figures.tex   Tue May 17 16:52:50 2022
...
% REQUIRED
\begin{abstract}
We introduce a novel eigenvalue algorithm for near-diagonal matrices \DIFaddbegin \DIFadd{inspired from Rayleigh-Schr\"odinger perturbation theory and }\DIFaddend termed \textit{iterative perturbative theory} (IPT). \DIFdelbegin \DIFdel{Built upon a ``perturbative" partitioning of the matrix into diagonal and off-diagonal parts, IPT can be used to compute one or all eigenpairs}\DIFdelend \DIFaddbegin \DIFadd{Contrary to standard eigenvalue algorithms which are either `direct' (to compute all eigenpairs) or `iterative' (to compute just a few), IPT computes any number of eigenpairs in parallel }\DIFaddend with the same basic iterative \DIFdelbegin \DIFdel{algorithm. Thanks to the high parallelism of matrix-matrix products, full-spectrum IPT shows excellent performance on multi-core processors and GPUs, with large speed-ups over standard direct methods (up to $\sim50$x with respect to LAPACK and MAGMA). For matrices which are not close to being diagonal but have well-separated eigenvalues, IPT can be be used to refine low-precision eigenpairs obtained by other methods. }\DIFdelend \DIFaddbegin \DIFadd{procedure, outperforming LAPACK and preconditioned iterative solvers respectively. }\DIFaddend We give sufficient conditions for linear convergence and demonstrate performance on dense and sparse test matrices\DIFdelbegin \DIFdel{. In a real-world application }\DIFdelend \DIFaddbegin \DIFadd{, including one }\DIFaddend from quantum chemistry\DIFdelbegin \DIFdel{, we find that single-eigenpair IPT performs similarly to the Davidson algorithm. }\DIFdelend \DIFaddbegin \DIFadd{. The code is available at }\url{http://github.com/msmerlak/IterativePerturbationTheory.jl}\DIFadd{.
...
\DIFdelbegin \DIFdel{Computing the eigenvalues and }\DIFdelend \DIFaddbegin 

\DIFadd{How can one compute the }\DIFaddend eigenvectors of a matrix \DIFaddbegin \DIFadd{$M$ }\DIFaddend that is already close to being diagonal\DIFdelbegin \DIFdel{(or diagonalizable in a knownbasis) is a classic problem with applications throughout science. }\textit{\DIFdel{Ab initio}} %DIFAUXCMD
\DIFdel{studies of electronic and nuclear structures, for instance, often involve Hamiltonian operators expressed in bases that nearly diagonalize them,  }\textit{\DIFdel{e.g.}} %DIFAUXCMD
\DIFdel{in self-consistent field or configuration interaction calculations \mbox{%DIFAUXCMD
\cite{ostlund1996}}\hspace{0pt}%DIFAUXCMD
. In classical electromagnetism, perturbative problems arise when the solution to a slightly deformed problem are sought, for instance when boundary conditions are shifted or optical properties are varied \mbox{%DIFAUXCMD
\cite{pozar1998}}\hspace{0pt}%DIFAUXCMD
. In mechanical engineering, one often wants to know the effect of slight deformations of stiffness or density on spectra, as in Rayleigh's pioneering analysis of vibrating strings \mbox{%DIFAUXCMD
\cite{rayleigh1894}}\hspace{0pt}%DIFAUXCMD
. More generally, we might know approximate eigenpairs through some method, and the problem is to refine them to a given precision. }\DIFdelend \DIFaddbegin \DIFadd{, or equivalently when approximate eigenvectors are known? In physics and chemistry, a standard approach, Rayleigh-Schr\"odinger perturbation theory \mbox{%DIFAUXCMD
\cite{rayleigh1894, kato1995}}\hspace{0pt}%DIFAUXCMD
,  attempts to compute these eigenvectors as power series in the `perturbation' $\Delta = M - D$, where $D$ is diagonal and $\Delta$ is small. The purpose of this paper is to show that a variant of this method, based on fixed-point iteration rather than power series expansion, can be the basis for a new kind of preconditioned eigenvalue algorithm. We stress from the outset that, like Rayleigh-Schr\"odinger perturbation theory, this algorithm (termed Iterative Perturbation Theory, IPT for short) has a limited scope: if $M$ is not sufficiently close to being diagonal, IPT will not converge. But in those cases---common in physical \mbox{%DIFAUXCMD
\cite{rayleigh1894, pozar1998} }\hspace{0pt}%DIFAUXCMD
or chemical \mbox{%DIFAUXCMD
\cite{ostlund1996} }\hspace{0pt}%DIFAUXCMD
applications---where $M$ is in fact near-diagonal, IPT proves much more efficient than established methods, sometimes by an order of magnitude or more.
}\DIFaddend 

Eigenvalue \DIFdelbegin \DIFdel{perturbation theory provides estimates and bounds for the variation of eigenpairs with respect to perturbations of the matrix \mbox{%DIFAUXCMD
\cite{sun1990}}\hspace{0pt}%DIFAUXCMD
.Here our aim is , instead, to compute these eigenpairs exactly (up to numerical error) , }\textit{\DIFdel{i.e.}} %DIFAUXCMD
\DIFdel{we are interested in eigenvalue algorithms for perturbative problems. 
In cases where only a single (or a few) eigenpairs are desired, iterative algorithms that take advantage of near-diagonality are available, }\DIFdelend \DIFaddbegin \DIFadd{algorithms are usually classified in `direct' and `iterative' methods \mbox{%DIFAUXCMD
\cite{demmel1997}}\hspace{0pt}%DIFAUXCMD
. These correspond to very different approaches to the eigenvalue problem. Direct methods, such as the QR and divide-and-conquer algorithms, reduce the the matrix into Hessenberg (or tridiagonal) form before they can compute complete set of eigenvectors.}\footnote{\DIFadd{One exception is Jacobi's algorithm for symmetric matrices, which annihilates off-diagonal elements with two-dimensional rotations. Because the complexity of Jacobi's algorithm is larger than that of tridiagonal reduction, this algorithm is not used in practice, except perhaps for small problems on GPUs.}} \DIFadd{Unfortunately, this initial step is difficult to parallelize, breaks any sparsity patterns in $M$, and has unfavorable (cubic) complexity. For this reason, direct algorithms are only applicable to small matrices with size $n \lesssim 10^4$. 
}

\DIFadd{Iterative methods, on the other hand, only aim to compute a few eigenpairs of $M$ through `matrix-free' applications of $M$ to vectors. Because they do not require reducing (or even explicitly forming) $M$, iterative methods can be applied to much larger problems than direct approaches, particularly when $M$ is sparse. Central to most of these methods is the Rayleigh-Ritz procedure: at each iteration, a small subspace is formed (}\DIFaddend e.g. \DIFdelbegin \DIFdel{Davidson-type subspace iteration methods \mbox{%DIFAUXCMD
\cite{Davidson_1975, fokkema1998jacobi, sleijpen2000jacobi, Morgan_2009} }\hspace{0pt}%DIFAUXCMD
or LOBPCG with a diagonal preconditioner \mbox{%DIFAUXCMD
\cite{knyazev2017recent}}\hspace{0pt}%DIFAUXCMD
. To obtain the complete set of eigenvectors of near-diagonal symmetric matrices, Jacobi's algorithm may be preferred to QR-based methods: the former's local quadratic convergence and good parallelizability are such that converged eigenvectors can sometimes be obtained in a fraction of the time required for tri-diagonalization}\DIFdelend \DIFaddbegin \DIFadd{by expansion from a smaller subspace), and the eigenvectors of the matrix projected to that subspace are computed}\DIFaddend . This is \DIFdelbegin \DIFdel{especially true on modern graphical processing units (GPUs), whose massive parallelism allow for extremely high flop rates that can sometimes offset the unfavourable complexity of Jacobi's algorithm \mbox{%DIFAUXCMD
\cite{demmel1997}}\hspace{0pt}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{the basis for classical Krylov methods (Arnoldi, Lanczos), but also for preconditioned methods which use an approximation of $M^{-1}$ to speed up convergence. The latter include Davidson methods (Generalized Davidson \mbox{%DIFAUXCMD
\cite{Davidson_1975, Morgan_2009}}\hspace{0pt}%DIFAUXCMD
, Jacobi-Davidson \mbox{%DIFAUXCMD
\cite{fokkema1998jacobi, sleijpen2000jacobi}}\hspace{0pt}%DIFAUXCMD
), Locally Optimal Block Preconditioned Conjugate Gradient (LOBPCG) \mbox{%DIFAUXCMD
\cite{knyazev2017recent}}\hspace{0pt}%DIFAUXCMD
, etc. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Here we introduce a novel iterative algorithm for the computation of one or all eigenpairs of a near-diagonal matrix. While more restricted in its applicability than some of the aforementioned methods, its performance tends to be higher, particularly on full-spectrum problems. This follows from our algorithm's simple linear-algebraic structure: each iteration consists of just one }\DIFdelend \DIFaddbegin \DIFadd{Our algorithm has a different structure. For starters, IPT makes no distinction between direct and iterative procedures: it computes any desired number of eigenpairs in parallel, from one to all of them, using the same basic iterative procedure. Second, IPT does not involve any reduction of the matrix, relying instead on repeated products $M\times Z$, where $Z$ is an $n\times k$ matrix when $k$ eigenpairs are requested. If $k = 1$, this is a }\DIFaddend matrix-vector \DIFdelbegin \DIFdel{multiplication (for one eigenpair) or one }\DIFdelend \DIFaddbegin \DIFadd{product, as in iterative methods; when $k = N$ (the full spectrum problem), this is a }\DIFaddend matrix-matrix \DIFdelbegin \DIFdel{multiplication (for all eigenpairs ). These operations are highly optimized in the Basic Linear Algebra Subprograms (BLAS Level 2 and 3 respectively), resulting in low execution times. Moreover, since matrix-matrix multiplication has }\DIFdelend \DIFaddbegin \DIFadd{product, with }\DIFaddend sub-cubic theoretical complexity \DIFdelbegin \DIFdel{$\mathcal{O}(N^{2.376})$, so does our algorithm, in contrast with classical direct algorithms which are $\mathcal{O}(N^{3})$ for the full spectrum problem. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Iterative perturbation theory (IPT) was inspired by the Rayleigh-Schr\"odinger (RS) perturbative expansion familiar from textbook quantum mechanics \mbox{%DIFAUXCMD
\cite{d1965}}\hspace{0pt}%DIFAUXCMD
. (The relationship between the two methods in the context of quantum physics will be discussed in a companion paper \mbox{%DIFAUXCMD
\cite{smerlak2021convergence}}\hspace{0pt}%DIFAUXCMD
. ) Its structure, however, is different: instead of seeking the perturbed eigenvectors as power series in the perturbation, we compute them iteratively, as fixed points of a quadratic equation . One consequence of this difference is that , unlike Rayleigh-Schr\"odinger expansions \mbox{%DIFAUXCMD
\cite{kato1995}}\hspace{0pt}%DIFAUXCMD
, the domain of convergence of our method as a scalar perturbation parameter is varied is not restricted to a disk in the complex plane bounded by exceptional points (values of the parameter such that the matrix is defective); instead, IPT shows for large perturbation the period-doubling route to chaos. }\DIFdelend \DIFaddbegin \DIFadd{$\mathcal{O}(n^{2.376})$. Third, IPT does not rely on the Rayleigh-Ritz procedure or external eigensolvers to solve a projected problem---it is completely self-contained. Fourth, existing methods normally distinguish between symmetric (or Hermitian) and non-symmetric problems. IPT does not assume or use symmetry and is equally efficient with non-symmetric problems. Finally, and most importantly in our view, perhaps, IPT reduces the eigenvalue equation into an elementary fixed-point problem. }\DIFaddend This means that \DIFdelbegin \DIFdel{a fixed point loses its stability and spawns a limit cycle of period two (two points alternated by the map). The limit cycle, in turn, loses stability by spawning a limit cycle of period four, which then spawns a limit cycle of period eight, and so on.
At a finite value of the perturbation parameter an accumulation of infinitely many of such period doublings happens culminating in the appearance of a chaotic attractor (see, for example, \mbox{%DIFAUXCMD
\cite{kuznetsov2004}}\hspace{0pt}%DIFAUXCMD
).
%DIF < We find that, in many cases, this allows for a larger convergence domain than RS perturbation theory.
}\DIFdelend \DIFaddbegin \DIFadd{the vast repertoire of methods developed in numerical for solving non-linear equations, including fixed-point acceleration methods, become directly relevant to the eigenvalue problem. We feel that this connection between non-linear analysis and numerical linear algebra is the most appealing aspect of IPT.
}\DIFaddend 

Our presentation starts with a reformulation of the eigenvalue equation as a fixed point equation for a quadratic map in complex projective space (\cref{fixed-points}). We then establish a sufficient condition for fixed point iteration to converge and illustrate its divergence for larger perturbations with a simple two-dimensional example (\cref{convergence}). \DIFaddbegin \DIFadd{An interesting aspect of IPT is its compabibility with fixed-point acceleration methods (}\cref{acceleration}\DIFadd{). }\DIFaddend Next, we consider the computational efficiency of our method on multi-CPU and GPU architectures using \DIFdelbegin \DIFdel{random test matrices}\DIFdelend \DIFaddbegin \DIFadd{test matrices, either sampled at random or drawn from chemistry applications }\DIFaddend (\cref{performance}). We \DIFdelbegin \DIFdel{then discuss one possible applications of our algorithm as a refinement method for eigenvectors (}%DIFDELCMD < \cref{applications}%%%
\DIFdel{). We }\DIFdelend conclude with some possible directions for future work.
...
\DIFaddbegin \begin{algorithm}[t]
    \caption{\DIFadd{One eigenvector of a near-diagonal matrix $M$}}\label{single_line_algo}
    \begin{algorithmic}[1]
        ...
        \STATE \DIFadd{Initialize $\mathbf{z} \leftarrow \mathbf{e}_i$
...
\DIFaddend We now further assume a partitioning
$$M=D+\Delta$$
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{One eigenvector of a near-diagonal matrix $M$}}%DIFAUXCMD
%DIFDELCMD < \label{single_line_algo}
%DIFDELCMD < \begin{algorithmic}[1]
%DIFDELCMD <  

%DIFDELCMD <     \STATE{Choose a partition $M = D + \Delta$ with $D$ diagonal}
%DIFDELCMD <     \STATE{Compute $\mathbf{g}_i \leftarrow ((D_{jj}-D_{ii})^{-1})_{j:j\neq i}$ and set $(\mathbf g_i)_i =0$}
%DIFDELCMD <     \STATE %%%
\DIFdel{Initialize $\mathbf{z} \leftarrow \mathbf{e}_i$
    }%DIFDELCMD < \WHILE{$\vert\mathbf z - \mathbf{f}_i(\mathbf z)\vert/\vert\mathbf z\vert >\eta$ for some tolerance $\eta$}        \STATE{$\mathbf z \leftarrow \mathbf{f}_i(\mathbf z)$ with $\mathbf{f}_i$ defined by \cref{single_line_dyn}}
%DIFDELCMD <     \ENDWHILE
%DIFDELCMD <     \STATE{Return eigenvector $\mathbf{z}$}
%DIFDELCMD <     \STATE{Return eigenvalue $\eigval_i = D_{ii} + (\Delta \mathbf{z})_i$}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend The same iterative technique can be used to compute all eigenvectors of $M$ in parallel (Algorithm \ref{full_spectrum_algo}). For this it suffices to bundle all $n$ candidate
...
\begin{algorithmic}[1]
    ...
    \DIFdelbegin %DIFDELCMD < \STATE{Return eigenvalues $\Eigval = D + \mathcal{D}(\Delta \Z)$}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \STATE{Return eigenvalues $\Eigval = \mathcal{D}(M \Z)$}
...
\subsection{Contrast with \DIFdelbegin \DIFdel{RS }\DIFdelend \DIFaddbegin \DIFadd{Rayleigh-Schr\"odinger }\DIFaddend perturbation theory}\label{IPT-RS}

It is interesting to constrast the present iterative method with conventional \DIFdelbegin \DIFdel{RS
}\DIFdelend \DIFaddbegin \DIFadd{Rayleigh-Schr\"odinger (RS)
}\DIFaddend perturbation theory, where the eigenvectors of a parametric matrix $M = D + \parameter \Delta$ are
...
\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \includegraphics[width=\textwidth]{plots/fig1}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Convergence of IPT in the two-dimensional
example }%DIFDELCMD < \cref{2by2}%%%
\DIFdelFL{. Left: In the
...
%DIFDELCMD < %%%
\DIFdelend \subsection{An explicit $2\times 2$ example}\label{2d-example}
...
\DIFdel{The variable $\mu$ here plays the role of a multiplier of a steady state. Either by successively computing resultants or by constructing a Groebner basis with the correct lexicographical order, one can exclude the variables $z_j$ from this system, which results in a single polynomial of two variables $(\parameter,\mu) \mapsto P(\parameter,\mu)$. This polynomial defines a complex 1-dimensional variety. The projection to the $\parameter$-plane of the real 1-dimensional variety defined by $\{P=0, \vert\mu\vert^2= 1\}$ corresponds to some curve $C$. A more informative way is to represent this curve as a complex function of a real variable $t$ implicitly defined by $P(\parameter,e^{it}) = 0$}\DIFdelend \DIFaddbegin \DIFadd{As a fixed-point problem, IPT is directly amenable to the broad set of fixed-point acceleration methods familiar from numerical analysis \mbox{%DIFAUXCMD
\cite{eyert1996comparative}}\hspace{0pt}%DIFAUXCMD
. Generally speaking, acceleration techniques aim to reduce the number of iterations required for convergence by combining several iterates of the function in each update}\DIFaddend . 

\DIFdelbegin \DIFdel{The curve $C$ is the locus where a fixed point of $\mathbf f_i$ have a multiplier on the unit circle. In particular, the fixed point that at $\parameter = 0$ corresponds to $z_i = 1$ and $z_j = 0$, $j \neq i$, loses its stability along a particular subset of this curve. The convergence domain of the iterative perturbation theory is the domain that is bounded by these parts of the curve and that contains $0$. It is possible to show that $C$ is a smooth curve with cusps (return points) , which correspond to the values of $\parameter$ such that $M$ has a nontrivial Jordan normal form (is non-diagonalizable) , see }%DIFDELCMD < \cref{appendix:cusps}%%%
...
\cite{kuznetsov2004}}\hspace{0pt}%DIFAUXCMD
, so these values of $\parameter$ can be found as a subset of the roots of the univariate polynomial $P(\parameter,1)$. Not all its roots generally correspond to cusps and fold bifurcations, though, as demonstrated in }%DIFDELCMD < \cref{appendix:examples}%%%
\DIFdel{. On the other hand, all the cusps/fold bifurcation points are among the $\parameter$-roots of the discriminant polynomial $\mathrm{Disc}_x\det(M - xI)$. These roots may correspond to an actually defective matrix $M$ (when two eigenvectors merge and $M$ acquires a nontrivial Jordan normal form) or a simple degeneration of its eigenvalues (when two eigenvalues become equal but $M$ retains a trivial Jordan normal form). Simple degenerations of eigenvalues are not related to cusps.
Thus, the cusp points of $C$ can be found as the intersection of the root sets of the two polynomials.
}\DIFdelend \DIFaddbegin \DIFadd{We experimented with the well-known Anderson acceleration \mbox{%DIFAUXCMD
\cite{Walker_2011} }\hspace{0pt}%DIFAUXCMD
and found that, indeed, IPT can be made to converge faster and in a larger domain that with simple Picard iteration. Here we use the more recent (but simpler) technique called Alternating Cyclic Extrapolation (ACX) \mbox{%DIFAUXCMD
\cite{lepage2021alternating}}\hspace{0pt}%DIFAUXCMD
. This method consists in replacing line 4,5 in }\cref{single_line_algo} \DIFadd{(or similarly in }\cref{full_spectrum_algo}\DIFadd{) by }\cref{ACX}\DIFadd{. As will see below, IPT with ACX acceleration (IPT-ACX) is a very efficient approach to near-diagonal eigenvalue problems.
}\DIFaddend 


\DIFaddbegin \begin{algorithm}[t]
    \caption{\DIFadd{Alternating Cyclic Extrapolation \mbox{%DIFAUXCMD
\cite{lepage2021alternating}}\hspace{0pt}%DIFAUXCMD
, with vector of orders $o = (3, 2, 3, 2, \cdots)$}}\label{ACX}
    \begin{algorithmic}[1]
        \STATE{$i = 1$}
...
\DIFaddend \section{Performance}\label{performance}

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \begin{center}
%DIFDELCMD < 

%DIFDELCMD < \includegraphics[width=.8\textwidth]{plots/fig5}
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Runtime vs. dimension $N$ for dense, non-symmetric perturbative matrices of the form }%DIFDELCMD < \cref{near-diagonal} %%%
\DIFdelFL{in single precision. Blue lines refer to IPT and orange lines to LAPACK's GEEV routine, implemented on 128 CPU cores (continuous lines) or on a Titan V GPU (dashed lines). Within its perturbative convergence domain, IPT is up to $32$ and $91$ times faster than GEEV, respectively.}}
	%DIFAUXCMD
%DIFDELCMD < \label{scaling}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We now compare the efficiency and accuracy of IPT to reference eigenvalues routines}\DIFdelend \DIFaddbegin \DIFadd{We investigated the performance of IPT with respect to classical algorithms using several test matrices}\DIFaddend . Our system consists of \DIFdelbegin \DIFdel{a dual AMD EPYC }\DIFdelend \DIFaddbegin \DIFadd{$2\times 64$ CPU cores (AMD Epyc }\DIFaddend 7702 \DIFdelbegin \DIFdel{with $128$ CPU cores using using LAPACK and SLEPc \mbox{%DIFAUXCMD
\cite{hernandez2005slepc} }\hspace{0pt}%DIFAUXCMD
and a NVidia Titan V GPUusing MAGMA \mbox{%DIFAUXCMD
\cite{Tomov_2010} }\hspace{0pt}%DIFAUXCMD
and Nvidia cuSOLVER.
Test matrices are of the form
}\begin{displaymath}\DIFdel{%DIFDELCMD < \label{near-diagonal}%%%
M = \textrm{diag}(n)_{1\leq n \leq N} + \parameter R
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $R$ is an array of standard normal random variables and $\parameter$ a positive parameter. We consider four cases: $R$ can either be dense or sparse (with density $50/N$, corresponding to $\sim 50N$ non-zero elements), and non-symmetric or symmetric (via $(R+R^T)/2$).For each case the time required to compute all $N$ eigenpairs in double precision is denoted $T_{\textrm{eig}}$, and the time to compute the lowest-lying eigenpair is denote $T_{\textrm{eigs}}$. As a reference we use the time for one matrix-matrix multiplication $T_{\textrm{mm}}$ or }\DIFdelend \DIFaddbegin \DIFadd{at 2.0 GHz) and }\DIFaddend one \DIFdelbegin \DIFdel{matrix-vector multiplication $T_{\textrm{mv}}$, respectively}\DIFdelend \DIFaddbegin \DIFadd{A100 NVidia GPU}\DIFaddend .
\DIFdelbegin \DIFdel{In all figures below we use continuous lines for experiments on the CPU and dashed lines for experiments on the GPU, and brackets indicate the wrapper we used to call the relevant eigenvalue routine. Unless specified otherwise, the error tolerance is $\eta = 100\epsilon$, where $\epsilon \simeq 2.2 \cdot 10^{-16}$ (resp. $\epsilon \simeq 1.2 \cdot 10^{-7}$) is machine epsilon in double (resp. single) precision. 
}\DIFdelend 

\DIFdelbegin \subsection{\DIFdel{Full spectrum}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Few eigenvalues}} \DIFadd{To compute a small subset of eigenvalues of a near-diagonal matrix, the reference algorithms are preconditioned iterative methods, with $D^{-1}$ as preconditioner. (Krylov methods are much slower for these problems.) We compared IPT to the symmetric eigensolver PRIMME (PReconditioned Iterative MultiMethod Eigensolver) \mbox{%DIFAUXCMD
\cite{stathopoulos2010primme}}\hspace{0pt}%DIFAUXCMD
, a package containing various implementations of Rayleigh Quotient Iteration (RQI), Generalized Davidson (GD, GD$+k$), Jacobi-Davidson (JDQR, JDQMR), and Locally Optimal Block Preconditioned Conjugate Gradient (LOBPCG).}\footnote{\DIFadd{We also tested SLEPc \mbox{%DIFAUXCMD
\cite{hernandez2005slepc} }\hspace{0pt}%DIFAUXCMD
but, on our single-node system, PRIMME was faster.}} \DIFadd{PRIMME also includes a "dynamic" mode with shifts dynamically between GD$+k$ and JDQMR in an attempt to minimize the number of matrix-vector multiplications. 
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \includegraphics[width=\textwidth]{plots/fig3}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{IPT vs. LAPACK for the complete diagonalization of perturbative matrices with increasing $\parameter$, in double precision. The largest speed-ups are obtained for the non-symmetric problem.}}
%DIFAUXCMD
%DIFDELCMD < \label{full_cpu}	
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We remind the reader that GD, JD and RQI all consist in expanding a small search subspace, orthonormalizing that subspace, and computing the eigendecomposition of $M$ in that subspace. Where they differ is how the subspace is expanded. GD uses the current residual vector $\mathbf r = \mathbf{f}(\mathbf z) - \mathbf z$ after preconditioning with $D^{-1}$. JD and RQI, by contrast, solve a linear ``correction'' equation (approximately or exactly, respectively) and expand the subspace with the correction vector; this inner iteration involves additional matrix-vector products per step compared to GD. We refer the reader to the documentation of PRIMME for implementation details. 
}\DIFaddend 

\DIFdelbegin \DIFdel{For the complete diagonalization problem we compare IPT to MATLAB's }\textbf{\DIFdel{eig}} %DIFAUXCMD
\DIFdel{function, which calls LAPACK's routines GEEV for non-symmetric matrices and SYEV for symmetric matrices; on the GPU MAGMA is used instead. 
}%DIFDELCMD < 

%DIFDELCMD <  \cref{full_cpu} %%%
\DIFdel{shows the corresponding CPU timings in the dense, sparse, symmetric and non-symmetric cases ($N=4096$). We make two observations. First, unlike LAPACK, our algorithm has a similar complexity with symmetric and non-symmetric matrices, leading to larger speed-ups in the latter case. Second, because it is based on matrix-matrix multiplication, IPT performs especially well on sparse matrices. This is another difference with standard full spectrum algorithms, which do not take advantage of sparsity. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As noted earlier, Hessenberg reduction is inefficient for matrices which are already near diagonal. In the next experiment we compare IPT with Nvidia's GPU implementation of Jacobi's algorithm, which does not rely on reduction and is known to have local quadratic convergence. In spite of thisadvantage, Jacobi (labeled SYEVJ in cuSOLVER) only proved faster than divide-and-conquer SYEVD for small matrices sizes; for $N=8182$ SYEVJ is slower than SYEVD for all values of $\parameter$ (}%DIFDELCMD < \cref{full_gpu}%%%
\DIFdel{, center and right panels).   
By contrast, our implementation of IPT in CUDA is faster than SYEVD for $\parameter \leq 0.05$. For dense, non-symmetric matrices, IPT is much faster than GEEV for all $\parameter$ compatible with convergence (}%DIFDELCMD < \cref{full_gpu}%%%
\DIFdelend \DIFaddbegin \DIFadd{For the implementation of IPT and ACX we used the Julia programming language \mbox{%DIFAUXCMD
\cite{Julia-2017}}\hspace{0pt}%DIFAUXCMD
. The code is freely available at \mbox{%DIFAUXCMD
\cite{IPT-code} }\hspace{0pt}%DIFAUXCMD
(for IPT) and \mbox{%DIFAUXCMD
\cite{paper-code} }\hspace{0pt}%DIFAUXCMD
(for the timings and figures). As already noted}\DIFaddend , \DIFdelbegin \DIFdel{top-left panel) .   
}\DIFdelend \DIFaddbegin \DIFadd{when using PRIMME we set $D^{-1}$ as preconditioner. Other than this, we used the default settings for all methods.   
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \includegraphics[width=\textwidth]{plots/fig2}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Timing of full spectrum routines on the GPU for dense perturbative matrices. Here IPT proves to be faster than both Divide and Conquer (SYEVD) and Jacobi (SYEVJ) when $\parameter$ is sufficiently small.}}
%DIFAUXCMD
%DIFDELCMD < \label{full_gpu}	
%DIFDELCMD < \end{figure*}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Our first example is from a real-world chemistry application. Full-configuration interaction (FCI) consists in computing the ground state (lowest-lying eigenvector) of the Hamiltonian operator of a molecule in a finite basis set \mbox{%DIFAUXCMD
\cite{ostlund1996}}\hspace{0pt}%DIFAUXCMD
. Because  molecular orbitals can be computed approximately using self-consisted field approximations, the corresponding (symmetric) Hamiltonian matrix is near-diagonal, and suitable eigensolvers must make use of this information. (Indeed, Davidson's diagonally-preconditioned algorithm \mbox{%DIFAUXCMD
\cite{Davidson_1975} }\hspace{0pt}%DIFAUXCMD
was introduced for this purpose.) 
}\DIFaddend 

\DIFdelbegin \subsection{\DIFdel{One eigenpair}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Next we consider the computation of just the lowest eigenpair of the matrices above, corresponding to the column $n=1$. Here performance comparison are based on the SLEPc library \mbox{%DIFAUXCMD
\cite{hernandez2005slepc, dalcin2011parallel}}\hspace{0pt}%DIFAUXCMD
, which includes the modern Krylov-Schur algorithm \mbox{%DIFAUXCMD
\cite{stewart2002krylov} }\hspace{0pt}%DIFAUXCMD
as well as preconditioned eigensolvers including Generalized Davidson (GD) \mbox{%DIFAUXCMD
\cite{Davidson_1975, morgan1986generalizations}}\hspace{0pt}%DIFAUXCMD
, Jacobi-Davidson \mbox{%DIFAUXCMD
\cite{sleijpen2000jacobi} }\hspace{0pt}%DIFAUXCMD
and , for symmetric problems, LOBPCG \mbox{%DIFAUXCMD
\cite{knyazev2017recent}}\hspace{0pt}%DIFAUXCMD
. When applying preconditioned eigensolvers , we use a diagonal (`Jacobi') preconditioner thattakes advantage of the near-diagonal structure }\DIFdelend \DIFaddbegin \DIFadd{Please note that, unlike standard iterative eigensolvers, IPT does not naturally target extremal eigenvalues; instead it computes perturbations the diagonal elements }\DIFaddend of $M$. \DIFdelbegin \DIFdel{For a discussion of Davidson-type methods in comparison with Lanczos, see \mbox{%DIFAUXCMD
\cite{morgan1986generalizations}}\hspace{0pt}%DIFAUXCMD
. Finally, we note that shift-and-invert methods were tested on the near-diagonal problems }%DIFDELCMD < \cref{near-diagonal}%%%
\DIFdel{, but proved less efficient than all methods reported here.}\DIFdelend \DIFaddbegin \DIFadd{In particular, to compute the lowest eigenvalue of a FCI matrix, we simply choose apply }\cref{single_line_algo} \DIFadd{for $i$ such that $M_{ii} = \min \textrm{diag}(M)$. 
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \cref{one} %%%
\DIFdel{presents our results. In all cases (dense, sparse, symmetric or non-symmetric) IPT runs much faster than ARPACK. Davidson, an algorithm designed for diagonally dominant matrices and used e. g. in quantum chemistry applications, provides a more meaningul reference point. Here we find that IPT is up to $\sim 5$x faster at small $\parameter$. This can be explained by the fact that , altough IPT requires a similar number of iterations as Davidson, each iteration is cheaper, with just one }\DIFdelend \DIFaddbegin \DIFadd{We computed the ground state of the FCI matrix for water ($\textrm{H}_2$O) in the minimal basis set ``sto-3g" (with $n = 441$ in this example). (To compute its elements we used the quantum chemistry package PySCF \mbox{%DIFAUXCMD
\cite{PYSCF}}\hspace{0pt}%DIFAUXCMD
.) }\cref{water} \DIFadd{(left) shows the convergence history (residual norm $\Vert \mathbf{f}(\mathbf{z}) - \mathbf{z}\Vert$ vs. number of }\DIFaddend matrix-vector \DIFdelbegin \DIFdel{multiplication per step. 
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure*}
%DIFDELCMD < \includegraphics[width=\textwidth]{plots/fig4}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{IPT vs. standard iterative algorithms for the lowest-lying eigenpair of perturbative matrices of the form }%DIFDELCMD < \cref{near-diagonal} %%%
\DIFdelFL{and increasing $\parameter$. Davidson-type methods with diagonal preconditioning outperform the Krylov-Schur algorithm on these problems, and IPT is yet faster when the perturbation is sufficiently small (or sparse).}}
%DIFAUXCMD
%DIFDELCMD < \label{one}	
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Parallelism}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Underlying the performance of IPT is the high parallelism of matrix-vector and, especially, matrix-vector multiplication. Such parallelism is not shared by Householder reflections, which is why eigenvalue routines based on Hessenberg or tri-diagonal reduction do not scale as well on }\DIFdelend \DIFaddbegin \DIFadd{products, matvecs) for various algorithms in PRIMME and for IPT(-ACX). In this example, both IPT and IPT-ACX converge to the desired tolerance (here $\eta = 10^{-12}$) with fewer matvecs that any of the PRIMME methods, with the latter about twice faster than the former. }\cref{water} \DIFadd{(right) shows the corresponding timings on our }\DIFaddend multi-core \DIFdelbegin \DIFdel{or GPU architectures. 
We illustrate this in  }%DIFDELCMD < \cref{parallel}%%%
\DIFdel{, where the time to diagonalize a dense, non-symmetric matrix with $\parameter = 0.1$ is plotted as a function of the number of active CPU cores. As expected from its simple structure, IPT parallelizes just as well as matrix-matrix multiplication (almost linearly in the number of cores). On large clusters with thousands of cores, we expect IPT will outperform GEEV by several orders of magnitude.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure*}
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[width=0.6\textwidth]{plots/fig6}
%DIFDELCMD < \end{center}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Parallel scaling. Unlike direct methods based on Hessenberg reduction (here GEEV for a near-diagonal matrix of the form }%DIFDELCMD < \cref{near-diagonal} %%%
\DIFdelFL{with parameters as indicated), IPT scales as well as BLAS (here matrix-matrix multiplication). As a result, the speed-up of IPT over GEEV is only limited by the number of CPU cores available.}}
%DIFAUXCMD
%DIFDELCMD < \label{parallel}	
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Accuracy}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Finally we compared the accuracy of IPT on the full spectrum problem with LAPACK via MATLAB's }\textbf{\DIFdel{eig}}%DIFAUXCMD
\DIFdel{. For this we used }\DIFdelend \DIFaddbegin \DIFadd{machine. 
}

\DIFadd{We performed the same calculations with a }\DIFaddend near-diagonal \DIFdelbegin \DIFdel{matrices of the form }%DIFDELCMD < \cref{near-diagonal} %%%
\DIFdel{with $N=1024$ in double precision varying $\parameter\in[10^{-4}, 0.2]$. For each $\parameter$ we ran IPT }\DIFdelend \DIFaddbegin \DIFadd{matrix considered by Morgan in his comparison of preconditioned eigensolvers \mbox{%DIFAUXCMD
\cite{morgan2000preconditioning}}\hspace{0pt}%DIFAUXCMD
: $M$ is the tri-diagonal, symmetric matrix with $M_{ii} = i$ }\DIFaddend and \DIFdelbegin \textbf{\DIFdel{eig}} %DIFAUXCMD
\DIFdel{to obtain the matrix of eigenvectors $\Z$ and corresponding eigenvalues $\Eigval=\textrm{diag}(\eigval_n)_{1\leq n\leq N}$. We then measured the accuracy of each routine through the residual error $\Vert M\Z-\Z \Eigval\Vert_F$, where $\Vert\cdot\Vert_F$ denotes the Frobenius norm. We find that IPT is more than an order of magnitude more accurate than LAPACK, with a median residual error of $4.4\cdot 10^{-11}$ and $6.4\cdot 10^{-10}$ respectively}\DIFdelend \DIFaddbegin \DIFadd{$M_{i, i+1} = M_{i, i -1} = 0.5$}\DIFaddend . \DIFaddbegin \cref{morgan} \DIFadd{shows the results. Here IPT requires many more matvecs than Davidson methods, but IPT-ACX does not, converging equally fast as the fastest PRIMME method for this example, LOBPCG. But since LOBPCG involves additional steps per iteration (diagonalization in the subspace), IPT-ACX proves faster overall (}\cref{morgan}\DIFadd{, right panel). 
}\DIFaddend 



\DIFdelbegin \section{\DIFdel{Applications}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{applications} %%%
\DIFdel{In this final section we consider two possible applications of IPT : as a mixed-precision eigenvalue algorithm (full spectrum problem), and as a potential competitor to Davidson in quantum chemistry (lowest-lying eigenvector). }\subsection{\DIFdel{A mixed-precision eigenvalue algorithm}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The condition that a matrix be near-diagonal of course severely restricts the applicability of IPT, although relevant examples exist in quantum chemistry, see below. However, IPT can be used more generally as a }\textit{\DIFdel{refinement}} %DIFAUXCMD
\DIFdel{method for well-conditioned eigenvalue problems. Consider a generic matrix $M$ with well separated eigenvalues, and assume given an approximation of its eigenmatrix $\Z_0$. For instance, we could compute $\Z_0$ using standard drivers in single precision, resulting in a single precision eigenmatrix $\Z_{0, s}$. Next, we convert this matrix to double precision, denoted $\Z_{0, d}$, and apply IPT to the near-diagonal matrix $M' = \Z_{0, d}^{-1}M \Z_{0, d}$. 
Once iterations have converged to a new eigenmatrix $\Z'$, we can rotate back and obtain accurate eigenvectors for $M$ as $\Z = \Z_{0,d} \Z'$. In practice the linear solve and matrix multiplication steps are fast, and so computing $\Z$ takes about the same time as the low-precision diagonalization $\Z_{0,s}$}\DIFdelend \DIFaddbegin \DIFadd{But the advantage of IPT vis-a-vis standard iterative methods becomes more apparent when we request several eigenpairs rather than just one. Unlike other methods, IPT computes eigenpairs in parallel (corresponding to the columns of $Z$) rather than sequentially, after deflation. The benefit of this parallelism is on display in }\cref{num_ev}\DIFadd{, where the timing to compute $k$ eigenvalues is compared across algorithms. For $k=50$, IPT-ACX is already several times faster than the fastest iterative method, and this gap only increases with $k$}\DIFaddend . 


%DIF < When applied to general random matrices (with independent normally distributed entries), this mixed-precision approach gives speed-ups over DGEEV (double-precision GEEV) of $\sim2-3$x at the cost of $\sim 10$x larger residual errors. (We did not attempt to lower these errors through balancing or else.) 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Obviously, this method cannot be used for ill-conditioned eigenvalue problems, because then $(i)$ diagonal gaps become large and IPT ceases to converge, and $(ii)$ the condition number of $\Z_{0, d}$ becomes large and accuracy is lost in the linear solve step. To measure the robustness of }\DIFdelend \DIFaddbegin \DIFadd{Finally we show in }\cref{fail} \DIFadd{how IPT fails when off-diagonal elements become too large compared to diagonal gaps. For this we consider a modification of }\DIFaddend the \DIFdelbegin \DIFdel{mixed-precision algorithm to eigenvalue clustering, we considered matrices }\DIFdelend \DIFaddbegin \DIFadd{Morgan matrix with $M_{ii} = i$ and $M_{i, i+1} = M_{i, i -1} = \varepsilon$ and increase $\varepsilon$. 
}




\subsection{\DIFadd{Full spectrum}} 

\DIFadd{Next we consider the problem of computing all eigenpairs of a large, dense matrix }\DIFaddend of the form 
\begin{equation}\DIFdelbegin %DIFDELCMD < \label{ill_cond}
%DIFDELCMD < %%%
\DIFdel{J_{\alpha} }\DIFdelend \DIFaddbegin \label{dense_matrix}
    \DIFadd{M }\DIFaddend = \DIFdelbegin \DIFdel{Q^{T}D_\alpha Q}%DIFDELCMD < \quad%%%
\textrm{\DIFdel{with}}%DIFAUXCMD
\DIFdel{\ D_\alpha=}\DIFdelend \textrm{diag}(\DIFdelbegin \DIFdel{10^{-\alpha n/N}}\DIFdelend \DIFaddbegin \DIFadd{i}\DIFaddend )\DIFdelbegin \DIFdel{_{1\leq n\leq N},
}\DIFdelend \DIFaddbegin \DIFadd{_{1 \leq i\leq n} + \varepsilon R_n
}\DIFaddend \end{equation}
where \DIFdelbegin \DIFdel{$\alpha$ is a parameter controlling the spacing between eigenvalues and $Q$ a random orthogonal matrix }\DIFdelend \DIFaddbegin \DIFadd{$R_n$ is a $n\times n$ matrix with uniformly distributed random entries in $[0,1]$. For a symmetric matrix with similar properties we consider $S = (M + M^t)/2$}\DIFaddend . 
\DIFdelbegin \DIFdel{For matrices of size $N=1024$ we found that IPT remained applicable up to $\alpha \simeq 4$, corresponding to a minimal spectral gap of $1.5\cdot10^{-6}$. (For this matrix }\textbf{\DIFdel{eigs}} %DIFAUXCMD
\DIFdel{failed to converge with the default parameters.) For all values of $\alpha$ below this threshold, we obtain acceptable residual errors ($\sim 5$x larger than DGEEV) and significant speed-ups over DGEEV ($2-3$x faster), see  }%DIFDELCMD < \cref{refinement}%%%
\DIFdel{.
}\DIFdelend 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Iterative refinement of eigenvectors of matrix $M$ with well-separated eigenvalues}}
%DIFAUXCMD
%DIFDELCMD < \begin{algorithmic}[1]   
%DIFDELCMD <     \STATE %%%
\DIFdel{Compute eigenvectors $\Z_{0,s}$ in single precision }\textit{\DIFdel{e.g.}} %DIFAUXCMD
\DIFdel{using SGEEV or SSYEV
    }%DIFDELCMD < \STATE %%%
\DIFdel{Set $\Z_{0,d}$ as $\Z_{0,s}$ in double precision
    }%DIFDELCMD < \STATE %%%
\DIFdel{Compute $M'\leftarrow \Z_{0,d}^{-1} M \Z_{0,d}$ using a linear solver in double precision
    }%DIFDELCMD < \STATE %%%
\DIFdel{Compute eigenvectors $\Z'$ and $\Eigval$ using IPT($M'$) }%DIFDELCMD < \STATE %%%
\DIFdel{Return eigenvectors $\Z = \Z_{0,d}\Z'$
    }%DIFDELCMD < \STATE %%%
\DIFdel{Return eigenvalues $\Eigval$
}%DIFDELCMD < \end{algorithmic}    
%DIFDELCMD < \end{algorithm}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Full configuration interaction}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{FCI}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Finally, we tested IPT on a real-world problem from quantum chemistry, the full configuration interaction (FCI)approach to }\textit{\DIFdel{ab initio}} %DIFAUXCMD
\DIFdel{electronic structure calculation \mbox{%DIFAUXCMD
\cite{ostlund1996}}\hspace{0pt}%DIFAUXCMD
. Given a molecule, its geometry (the position of nuclei) and a basis set, FCI provides the most accurate estimate of the energy and wavefunction of electrons available. Formally, an approximation of the eigenvectors of the Hamiltonian matrix is first computed with self-consistent field (Hartree-Fock)methods; in that basis the Hamiltonian matrix $H$ is }\DIFdelend \DIFaddbegin \DIFadd{Dense eigenproblems are normally solved with the QR or divide-or-conquer algorithm, implemented in the Linear Algebra PACKage (LAPACK) under the name (GEEV). On NVidia GPUs, the CUSOLVER \mbox{%DIFAUXCMD
\cite{c1998} }\hspace{0pt}%DIFAUXCMD
package provides an efficient version of divide-and-conquer for symmetric matrices (SYEVD). As already emphasized, these methods do not take advantage of any particular structure in $M$ except symmetry; in particular they do not perform better on sparse or }\DIFaddend near-diagonal \DIFdelbegin \DIFdel{(as well as symmetric), and FCI proceeds by computing iteratively the eigenvector $z_0$ with smallest eigenvalue $\eigval_0$, usually using the Davidson algorithm already mentioned above. 
Because the dimension $N$ of the Hamiltonian matrix $H$ for $n_e$ electrons distributed in $n_b$ basis sets is $(n_b\ \textrm{choose}\ n_e)$ (hence grows factorially with system size), the bottleneck in iterative FCI computations is the matrix-vector product $H\mathbf z$. (Fig. \ref{H2O_matrix} shows the sparsity pattern of a small FCI matrix.)}\DIFdelend \DIFaddbegin \DIFadd{matrices than on general matrices. 
}\DIFaddend 

\DIFdelbegin \DIFdel{In order to reduce the number of iterations as much as possible, we applied Anderson acceleration to the fixed point iteration $\mathbf z \leftarrow \mathbf f_0(\mathbf z)$ in Algorithm \ref{single_line_algo}. We recall that Anderson acceleration with memory $m$ replaces the current iterate $\mathbf z^{(k)}$ with a convex combination of $m_k=\min\{m,k\}$ previous iterates, with weights $\alpha^{(k-j)}$ ($0\leq j \leq m_k $) defined by a least-squares minimization problem (see \mbox{%DIFAUXCMD
\cite{Walker_2011} }\hspace{0pt}%DIFAUXCMD
and references therein) . We compared IPT with Anderson acceleration (with memory $m=5$)with the implementation of the Davidson algorithm provided in the open-source quantum chemistry suite PySCF \mbox{%DIFAUXCMD
\cite{PYSCF} }\hspace{0pt}%DIFAUXCMD
(using the function }\textbf{\DIFdel{pyscf.lib.davidson}}%DIFAUXCMD
\DIFdel{) ; for a python implementation of Anderson acceleration we used the code from Ref. \mbox{%DIFAUXCMD
\cite{zhang2020globally}}\hspace{0pt}%DIFAUXCMD
. The convergence criterion was based on the residual, namely $\Vert H\mathbf z_0 - \eigval_0 \mathbf z_0\Vert \leq 10^{-8}$. }%DIFDELCMD < 

%DIFDELCMD <  
%DIFDELCMD <  \begin{figure*}
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[scale = 0.4]{plots/H20_sto-3g.pdf}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Sparsity pattern of a small FCI matrix, here for $\mathrm{H}_2\mathrm{O}$ in the minimal basis set sto-3g.}}
%DIFAUXCMD
%DIFDELCMD < \label{H2O_matrix}
%DIFDELCMD < \end{figure*}
%DIFDELCMD < \begin{center}
%DIFDELCMD < 

%DIFDELCMD <  \begin{tabular}[t]{|c c c c c|} 
%DIFDELCMD <  \hline
%DIFDELCMD <  %%%
\DIFdel{Molecule }%DIFDELCMD < & %%%
\DIFdel{Basis set }%DIFDELCMD < & %%%
\DIFdel{$N$ }%DIFDELCMD < & %%%
\DIFdel{($K$}\DIFdelend \DIFaddbegin \DIFadd{We timed multi-CPU and GPU implementations of IPT to these methods for matrices }\cref{dense_matrix} \DIFadd{with $\varepsilon = 10^{-2}$, double precision ($\eta = 10^{-12}$)}\DIFaddend , \DIFdelbegin \DIFdel{$T$) Davidson }%DIFDELCMD < &  %%%
\DIFdelend \DIFaddbegin \DIFadd{and increasingly large dimension $n$. }\DIFaddend (\DIFdelbegin \DIFdel{$K$, $T$) IPT-AA }%DIFDELCMD < \\
%DIFDELCMD <  \hline
%DIFDELCMD <  %%%
\DIFdel{He }%DIFDELCMD < & %%%
\DIFdel{cc-pvqz }%DIFDELCMD < & %%%
\DIFdel{900 }%DIFDELCMD < & %%%
\DIFdelend \DIFaddbegin \DIFadd{Both IPT and IPT-ACX diverge for $\varepsilon \gtrsim .05$) compared with these reference routines (}\cref{timings}\DIFadd{). The result is that IPT is up to 2 orders of magnitude more efficient than the non-symmetric routine GEEV, and also more efficient than the }\DIFaddend (\DIFdelbegin \DIFdel{9, 0.098) }%DIFDELCMD < & %%%
\DIFdel{(8, 0.055)  }%DIFDELCMD < \\ 
%DIFDELCMD <  \hline
%DIFDELCMD <  %%%
\DIFdel{Be }%DIFDELCMD < & %%%
\DIFdel{cc-pvtz }%DIFDELCMD < & %%%
\DIFdel{189225 }%DIFDELCMD < & %%%
\DIFdel{(13, 1.82) }%DIFDELCMD < & %%%
\DIFdel{(13, 1.82) }%DIFDELCMD < \\
%DIFDELCMD <   \hline
%DIFDELCMD < %%%
\DIFdel{$\mathrm{H}_2\mathrm{O}$ }%DIFDELCMD < & %%%
\DIFdel{sto-6g }%DIFDELCMD < & %%%
\DIFdel{441 }%DIFDELCMD < & %%%
\DIFdel{(9, 0.003) }%DIFDELCMD < & %%%
\DIFdel{(8, 0.002) }%DIFDELCMD < \\
%DIFDELCMD <  \hline
%DIFDELCMD < %%%
\DIFdel{$\mathrm{H}_2\mathrm{O}$ }%DIFDELCMD < & %%%
\DIFdel{6-31G }%DIFDELCMD < & %%%
\DIFdel{1656369 }%DIFDELCMD < & %%%
\DIFdel{(18, 5.36) }%DIFDELCMD < & %%%
\DIFdel{(20, 4.75) }%DIFDELCMD < \\
%DIFDELCMD <  \hline
%DIFDELCMD <  %%%
\DIFdel{LiH }%DIFDELCMD < & %%%
\DIFdel{6-31g }%DIFDELCMD < & %%%
\DIFdel{3025 }%DIFDELCMD < & %%%
\DIFdel{(12, 0.028) }%DIFDELCMD < & %%%
\DIFdel{(12, 0.027) }%DIFDELCMD < \\
%DIFDELCMD <   \hline
%DIFDELCMD <  %%%
\DIFdel{LiH }%DIFDELCMD < & %%%
\DIFdel{cc-pvtz }%DIFDELCMD < & %%%
\DIFdel{894916 }%DIFDELCMD < & %%%
\DIFdel{(15, 39.95) }%DIFDELCMD < & %%%
\DIFdel{(15, 41.86) }%DIFDELCMD < \\
%DIFDELCMD <   \hline
%DIFDELCMD < \end{tabular}\label{fci_table}
%DIFDELCMD < \captionof{table}{Full configuration interaction calculation on small molecules, using different geometries and basis sets. With Anderson acceleration (memory $m=5$), IPT is equally efficient as the Davidson algorithm which is standard in the field. Here $K$ and $T$ denote the number of matrix-vector evaluations and total CPU time (in s) respectively.}
%DIFDELCMD < \end{center}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{much faster) symmetric routine SYEVD. 
}\DIFaddend 


\DIFdelbegin \DIFdel{As showed in Table 1, we find that IPT-AA converges for realistic FCI problems with a number }\DIFdelend \DIFaddbegin \DIFadd{The reason for this enhanced performance on near-diagonal problems is again due to the better parallelism of IPT. For the full-spectrum problem, IPT relies on matrix-matrix products which are efficiently parallelized with BLAS 3; the Hessenberg reduction with underlies direct methods does not benefit from such parallelism. }\cref{parallel} \DIFadd{shows the timing of eigendecompositions vs. that }\DIFaddend of \DIFdelbegin \DIFdel{iterations and timing comparable to Davidson. One conceptual difference between the two methods, however, is that IPT is self-contained, whereas Davidson relies on other eigenvalue algorithms to obtain Ritz values}\DIFdelend \DIFaddbegin \DIFadd{matrix-matrix multiplications as a function of the number of CPU cores used. While this ratio increases with GEEV (indicating worse parallelism than BLAS 3), it does not with IPT (as expected)}\DIFaddend .  



\section{Discussion}

We have presented a new eigenvalue algorithm for near-diagonal matrices, be them symmetric or non-symmetric, dense or sparse. IPT can be applied to obtain a single perturbed eigenvector \DIFdelbegin \DIFdel{, with performance comparable to }\DIFdelend \DIFaddbegin \DIFadd{and often outperforms }\DIFaddend state-of-the-art preconditioned eigensolvers. IPT can also be applied to the full spectrum problem; in that case IPT benefits from a lower theoretical complexity and lower runtime than \DIFdelbegin \DIFdel{standard methods }\DIFdelend \DIFaddbegin \DIFadd{dense eigensolvers }\DIFaddend based on Hessenberg reduction. The largest \DIFdelbegin \DIFdel{speed-ups are }\DIFdelend \DIFaddbegin \DIFadd{speed-ups---up to two-orders of magnitude---are }\DIFaddend obtained for non-symmetric, full spectrum problems. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We emphasize that the near-diagonality condition is quite restrictive: off-diagonal matrix elements must be small compared to diagonal spacings. While their are important applications with this property (e.g. full configuration interaction), it is fair to say that }\DIFdelend \DIFaddbegin \DIFadd{To our knowledge, }\DIFaddend IPT is \DIFdelbegin \DIFdel{more akin to a refinement procedure than a full-fledged eigenvalue algorithm. 
Even so IPT can be useful: using a mixed-precision approach we computed the eigenvectors of a general dense matrix with random entries to double precision in a fraction of the time required by DGEEV. 
}\DIFdelend \DIFaddbegin \DIFadd{the first full-spectrum eigensolver that is able to take initial guesses into account. 
}\DIFaddend 
...
improve the convergence of the RS series \cite{Surj_n_2004}. Finally, we saw that the convergence of IPT can be improved using \DIFdelbegin \DIFdel{Anderson acceleration , which can be viewed as an }\textit{\DIFdel{ad hoc}} %DIFAUXCMD
\DIFdel{non-linear Krylov subspace method}\DIFdelend \DIFaddbegin \DIFadd{acceleration methods}\DIFaddend . It would be interesting to see \DIFdelbegin \DIFdel{whether there exists an optimal combination of previous iterates to find the fixed points of the IPTmap}\DIFdelend \DIFaddbegin \DIFadd{how far such ideas can go in extending the scope of IPT}\DIFaddend .



\DIFdelbegin %DIFDELCMD < \begin{figure*}
%DIFDELCMD < \includegraphics[width=\textwidth]{plots/fig7}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{IPT as a refinement algorithm for ill-conditioned problems }%DIFDELCMD < \cref{ill_cond} %%%
\DIFdelFL{with increasingly small eigengap. Combining single-precision GEEV with IPT gives eigenvectors with acceptable accuracy at a fraction of the computational cost of double-precision GEEV.}}
%DIFAUXCMD
%DIFDELCMD < \label{refinement}	
%DIFDELCMD < \end{figure*}
%DIFDELCMD < 
...
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[width=0.70\columnwidth]{plots/figSIn1}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < {%%%
\DIFdelFL{The convergence domain on the~\(\parameter\)-plane for the first
...
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[width=0.70\columnwidth]{plots/figSIn2}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < {{%%%
\DIFdelFL{Same as ~}%DIFDELCMD < {\cref{473965}} %%%
\DIFdelFL{for the second
...
%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < \begin{center}
%DIFDELCMD < \includegraphics[width=0.70\columnwidth]{plots/figSIn3}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < {{%%%
\DIFdelFL{Same as ~}%DIFDELCMD < {\cref{473965}} %%%
\DIFdelFL{for the third
eigenvector~\(\mathbf z_3\) (the third column of~\(\Z\)) .
}%DIFDELCMD < {\label{824432}}%%%
%DIF < 
%DIFDELCMD < }%%%
%DIF < 
%DIFDELCMD < }%%%
